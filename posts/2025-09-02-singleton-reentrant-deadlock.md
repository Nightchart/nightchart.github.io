---
title: 一次启动自死锁：单例重入引发的崩溃排查实录
slug: singleton-reentrant-deadlock
date: 2025-09-02
description: 程序一启动就挂死：无崩溃、无日志、CPU 闲着。复盘一次单例构造期重入引发的死锁，从线程栈定位到三层修复，附一份防复发清单。
tags: C++, 单例, 死锁, 并发, 排查实录
---
...
![构造期重入调用链示意](assets/fig4-deadlock-callchain.svg)
...
![magic static 守卫：并发安全，重入死锁](assets/fig5-guard-reentry.svg)
...
![修复前后对比](assets/fig6-fix-compare.svg)
...
按计划，下一篇回到标准主线《S-100 体系地图：S-101/102/124/125/127/129/131 各是什么》；踩坑实录这个系列也会继续——真实的工程问题，比标准条文好消化。
