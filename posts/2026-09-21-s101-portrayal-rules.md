...
上一篇[拆 PC 分发件](s101-portrayal-lookup.html)时留了个坑：目录里登记着 215 个规则文件的"引用"，但解开包只看到数据——符号怎么画有 XML，颜色怎么配有 colorProfile，**唯独没有"什么条件下画什么"的逻辑**。S-52 时代的 Look-up 表好歹是一张能读的表，S-101 把它变成了 215 个 Lua 文件——约 200 个物标规则，外加 7 个 `PortrayalAPI`、`S100Scripting` 之类的框架与公共文件。
