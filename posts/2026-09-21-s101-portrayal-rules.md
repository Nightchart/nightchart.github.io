...
上一篇[拆 PC 分发件](s101-portrayal-lookup.html)时留了个坑：目录里登记着 215 个规则文件的"引用"，但解开包只看到数据——符号怎么画有 XML，颜色怎么配有 colorProfile，**唯独没有"什么条件下画什么"的逻辑**。S-52 时代的 Look-up 表好歹是一张能读的表，S-101 把它变成了 215 个 Lua 文件——208 个物标规则，外加 7 个 `PortrayalAPI`、`S100Scripting` 之类的框架与公共文件。

这些文件不在展示用的分发包里，而是以 CSP（Conditional Symbology Procedure，条件制图程序）源码的形式随产品规范发布——IHO 官网上各产品规范的 Portrayal 附件里就能下到。这件事的分量值得单独说一句：**条件制图从"给人读的文档"变成了"给机器跑的程序"**。S-52 PresLib 时代，PL 3.4 / PL 4.0 里的条件逻辑是伪代码，每家渲染器自己翻译成 C++——翻译过程就是实现分歧的温床。S-101 干脆把官方实现（Lua）连同文档一起发，你要做的不是"照着文档写"，而是"把这段 Lua 嵌进去跑"。

这篇把其中的代表拆开看：一个最小的，一个最复杂的，最后看一族不用 Lua 的。
...
第一个条件就值得停一下：**属性直接改写符号**。锚泊船装的是危险品（categoryOfCargo 含 7），符号从普通锚泊换成危险品锚泊标——船员扫一眼符号就知道这片锚地里混着危险品船。这行代码在 S-52 里对应 Look-up 表里的一条带条件的行，语义没变，只是载体从表行变成了 if。
...
为什么两套并存？XSLT 是 S-52 预库时代就确立的技术路线，这批产品规范延续了它；S-101 较新，规则换成了 Lua。对我们做渲染器的人，实际含义是：**解析器要么支持两种规则引擎，要么在接入不同产品规范前先做一层转换**。我们的做法是工具里双引擎并列：同一个 S-131 分发包实测吃下 41 个 Lua 规则，S-123 分发包实测吃下 26 个 XSLT 规则文件（87 个 XML、657 个符号，覆盖 11 个物标），两条管道各自出渲染预览。
