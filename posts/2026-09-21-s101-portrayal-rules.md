...
DEPARE03 遍历自己每条共享边，把共边的要素捞出来分堆：共边的是 `DepthContour`？看它的深度值是不是恰好等于安全等深线（`loc_safety`）。是 `LandArea`、`UnsurveyedArea`？那要看对方的水线效果（`waterLevelEffect`）——陆地和未测量区贴着的边按 unsafe 处理。连内陆水域都有特判：River、Lake、Canal、DockArea 算一组，还要看线性建筑物的水线效果属性。
